...
chrome.alarms.onAlarm.addListener(alarm => {
  chrome.storage.local.get(
    ["date", "nextSelectedWashDelay", "washDelay", "tabMode", "isPaused"],
    storage => {
      if (alarm.name == "alarm" + storage.date && storage.isPaused == false) {
        ...
        chrome.storage.local.set(
          { washDelay: storage.nextSelectedWashDelay },
          () => {
            clearAndCreateAlarm(
              storage.nextSelectedWashDelay,
              storage.nextSelectedWashDelay
...
chrome.runtime.onConnect.addListener(port => {
  port.onMessage.addListener(response => {
    if (response.resetTime) {
      chrome.storage.local.get(
        ["nextSelectedWashDelay", "isPaused", "pausedCount"],
        storage => {
          if (storage.isPaused)
            chrome.storage.local.set({ isPaused: false, pausedCount: 0 });
          clearAndCreateAlarm(
            storage.nextSelectedWashDelay,
            storage.nextSelectedWashDelay
          );
